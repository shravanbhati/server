...
function show_kana() {	
	...
	if(font == 'default') {
		...
		document.getElementById('kana').innerHTML = '<img class="inverse" src="https://raw.githubusercontent.com/djtguide/djtguide.github.io/main/learn/kana/fonts/' + font + '/' + cur_kana + '.png" />';
	}
...
function play_sound() {
    ...
    audio = new Audio('https://raw.githubusercontent.com/shravanbhati/server/main/YukiNihongo/audio/' + cur_reading + '.mp3');
    audio.play();
    document.getElementById('input_box').focus();
}

function play_other(file) {
	var audio = new Audio('https://raw.githubusercontent.com/shravanbhati/server/main/YukiNihongo/audio/' + file + '.mp3');
	audio.play();
}

function stroke_order() {
	document.getElementById('kana').innerHTML = '<img src="https://raw.githubusercontent.com/shravanbhati/server/main/stroke/' + cur_kana + '.gif" id="stroke" />';
	document.getElementById('input_box').focus();
}
...
function checkAll(classname) {
    ...
    // Call collect to update selected kana
    collect();
}
...
function uncheckAll(classname) {
  ...
  //alt
// Check all alternate Hiragana
function checkAllAltHiragana() {
    var checkboxes = document.getElementsByClassName('hiragana-alt');
    for (var i = 0; i < checkboxes.length; i++) {
        checkboxes[i].checked = true;
    }
  collect();
...
// Uncheck all alternate Hiragana
function uncheckAllAltHiragana() {
    var checkboxes = document.getElementsByClassName('hiragana-alt');
    for (var i = 0; i < checkboxes.length; i++) {
        checkboxes[i].checked = false;
    }
...
// Check all alternate Katakana
function checkAllAltKatakana() {
    var checkboxes = document.getElementsByClassName('katakana-alt');
    for (var i = 0; i < checkboxes.length; i++) {
        checkboxes[i].checked = true;
    }
  collect();
...
// Uncheck all alternate Katakana
function uncheckAllAltKatakana() {
    var checkboxes = document.getElementsByClassName('katakana-alt');
    for (var i = 0; i < checkboxes.length; i++) {
        checkboxes[i].checked = false;
    }
...
function toggleOptions() {
    const optionsDiv = document.querySelector('.kanahideshow');
    
    // Explicitly set the initial state to 'none' if it's not already defined
    if (!optionsDiv.style.display) {
